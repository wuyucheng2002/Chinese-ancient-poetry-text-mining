    ...
    for epoch in range(epochs):
        for src, tgt, label, src_valid_len, tgt_valid_len in train_dataiter:
            ...
            if global_step and global_step % eval_step == 0:
                dev_bleu_score = eval(src_bert, mt_model, src_vocab, tgt_vocab, dev_dataiter, logger, ctx=ctx)
                if not os.path.exists(params_save_path_root):
                    os.makedirs(params_save_path_root)
                model_params_file = params_save_path_root + "src_bert_step_{}.params".format(global_step)
                src_bert.save_parameters(model_params_file)
                logger.info("{} Save Completed.".format(model_params_file))

                model_params_file = params_save_path_root + "mt_step_{}.params".format(global_step)
...
def eval(src_bert, mt_model, src_vocab, tgt_vocab, dev_dataiter, logger, ctx):
    ...
    for src, _, label, src_valid_len, label_valid_len in tqdm(dev_dataiter):
        ...
        predict_valid_len = nd.sum(nd.not_equal(predicts, tgt_vocab(tgt_vocab.padding_token)), axis=-1).asnumpy().tolist()
    ...
    logger.info("refer :{}".format(" ".join(references[0]).replace(EOS, "[EOS]").replace(tgt_vocab.padding_token, "")))
    logger.info("hypoth:{}".format(" ".join(hypothesis[0]).replace(EOS, "[EOS]")))
...
def main(args):
    ...
    # build trainer
    finetune_trainer = gluon.Trainer(src_bert.collect_params(), args.optimizer, {"learning_rate": args.finetune_lr})
    trainer = gluon.Trainer(mt_model.collect_params(), args.optimizer, {"learning_rate": args.train_lr})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="bert2transformerOnMachineTranslation")
    parser.add_argument("--train_data_path", type=str, default='../诗歌翻译2.txt')
    parser.add_argument("--eval_data_path", type=str, default='../诗歌翻译3.txt')
    ...
    parser.add_argument("--src_bert_load_path", type=str, default='../checkpoints1/src_bert_step_140000.params')
    parser.add_argument("--mt_model_load_path", type=str, default='../checkpoints1/mt_step_140000.params')
    ...
    parser.add_argument("--params_save_path_root", type=str, default="../checkpoints/")
    parser.add_argument('--warmup_ratio', type=float, default=0.1, help='ratio of warmup steps that linearly '
                        'increase learning rate from 0 to target learning rate. default is 0.1')
